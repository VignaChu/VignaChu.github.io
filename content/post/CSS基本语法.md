...
| 值              | 生成盒子类型    | 典型表现            | 使用场景              |
| -------------- | --------- | --------------- | ----------------- |
| `block`        | 块级盒子      | 独占一行、可设宽高       | `div`/`p`/`h1` 默认 |
| `inline`       | 行内盒子      | 不换行、宽高无效        | `span`/`a` 默认     |
| `inline-block` | 行内块       | 不换行、**可设宽高**    | 横向按钮/标签组          |
| `none`         | **不生成盒子** | 不占位、不可见         | 隐藏元素（JS 切换显示）     |
| `flex`         | 弹性容器      | 子元素变成弹性项目       | 一维布局（导航、居中）       |
| `inline-flex`  | 行内弹性容器    | 容器本身不换行         | 小部件横向排列           |
| `grid`         | 网格容器      | 子元素变成网格项目       | 二维布局（整页、表单）       |
| `inline-grid`  | 行内网格容器    | 容器本身不换行         | 小表格/卡片组           |
| `table`        | 块级表格      | 类似 `<table>`    | 快速把 div 变表格       |
| `inline-table` | 行内表格      | 表格不换行           | 少见                |
| `list-item`    | 块级+列表标记   | 自动生成 `::marker` | `li` 默认           |
| `contents`     | **盒子被丢弃** | 子元素直接上浮一层       | 消除中间 wrapper（实验性） |
